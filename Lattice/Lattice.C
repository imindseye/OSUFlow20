...
// Tom Peterka, 12/8/08
//
Lattice::Lattice(int xlen, int ylen, int zlen, int min_t, int max_t, int ghost, int np) {
  ...
  for (int j = 0; j < np; j++)  {
    ...
    for (int i = 0; i < 6; i++){
      ...
      if ((parts[j].SendPoints[i] = (float *)malloc(4 * sizeof(float))) == NULL)
	Error("Error: Lattice() cannot allocate memory for SendPoints\n");
      if ((parts[j].RecvPoints[i] = (float *)malloc(4 * sizeof(float))) == NULL)
...
void Lattice::GetPartitions(int proc, int**p_list, int& num) {
  ...
  num = cnt; 
  (*p_list) = new int[cnt]; // maybe this can be implemented using List? 
  cnt = 0; 
...
void Lattice::InsertSeed(int i, int j, int k, VECTOR4 p) {
...
// Tom Peterka, 12/4/08
//
void Lattice::PostPoint(int myrank, VECTOR4 p, int neighbor) {
...
void Lattice::PrintPost(int myrank) {
  ...
  for (i = 0; i < 6; i++) {
	...
	fprintf(stderr, "%.3f\t%.3f\t%.3f\t%.3f\n",
            ...
            parts[myrank].SendPoints[i][3 * j + 3] 
            );
    }
...
void Lattice::PrintRecv(int myrank) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    parts[myrank].RecvPoints[i][3 * j + 3]
    );
    }
...
// Tom Peterka, 12/4/08
//
void Lattice::GetRecvPts(int myrank, VECTOR4 *ls) {
